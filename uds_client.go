    ...
    for numberOfMessages:= 1; numberOfMessages <= maxNumberOfMessages; numberOfMessages = numberOfMessages * 2 {
        for numOfThreads:= 1; numOfThreads <= maxConcurrency; numOfThreads = numOfThreads * 2 {
            for dataSize := 2; dataSize <= maxSize; dataSize = dataSize * 2 {
                ...
                record := [] string {strconv.Itoa(connectionPoolSize), strconv.Itoa(numOfThreads), strconv.Itoa(numberOfMessages), strconv.Itoa(dataSize), strconv.FormatInt(t1.Sub(t0).Nanoseconds() / (int64(numberOfWrites) * 1000), 10)}
                writer.Write(record)
                fmt.Printf("conns:%d, Conc:%d, noOfMsgs:%d, size:%d bytes,  AvgLat:%d us.\n", connectionPoolSize, numOfThreads, numberOfMessages, dataSize, t1.Sub(t0).Nanoseconds() / (int64(numberOfWrites) * 1000))
            }
        }
    }
}
